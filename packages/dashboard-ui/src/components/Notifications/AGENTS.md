...
Also follow `../../AGENTS.md` for package boundaries, visual verification, and
app-shell ownership.
...
- The root app-shell provider is the page-level notification sink. It should use `mode="multiple"` and render `NotificationFlashbar`, because the flashbar is the only dashboard surface that should show multiple notifications at once.
...
- `Form` clears nearest-provider notifications on submit. This clears stale form-local API errors while leaving root flashbar messages untouched.
- If a form-related RTK Query hook is called outside the `<Form>` component, lift `FormProvider` or a thin product-specific provider wrapper above the hook owner so automatic errors are still caught by the form-local provider.
