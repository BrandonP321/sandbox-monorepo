...
- `getQuery` and `getMutation` accept optional notification options at wrapper creation time. Use them for API-level success messages, error titles, rare user-facing error-message overrides, or `displayError: false` when a caller intentionally renders the API error elsewhere.
