...
describe("lambda handler", () => {
  ...
  it("routes topic timeline requests through the standard app router", async () => {
    const event = {
      rawPath: signalTrackerRoutes.listTopicTimeline.path,
      body: JSON.stringify({ topicId: " ", limit: 1 }),
      requestContext: {
        http: {
