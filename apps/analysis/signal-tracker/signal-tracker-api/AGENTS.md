...
- Topic delete is intentionally a permanent hard delete of the topic row. Use `archive-topic` for reversible/non-destructive topic lifecycle behavior, and do not add topic-level `deleted` status or `deletedAt` fields unless a future product decision explicitly changes this.
- Entry lifecycle is separate from topic lifecycle. Do not remove entry `deleted` status or `deletedAt` fields when working on topic delete/archive cleanup.
